...
const PROPERTY_PATTERNS = {
  ...
  meltingPoint: {
    ...
    sort: (a, b) => {
      const av = parseFloat(a.meltingPoint),
        bv = parseFloat(b.meltingPoint);
  ...
  boilingPoint: {
    ...
    sort: (a, b) => {
      const av = parseFloat(a.boilingPoint),
        bv = parseFloat(b.boilingPoint);
      return (isNaN(bv) ? -Infinity : bv) - (isNaN(av) ? -Infinity : av);
    },
  },
  electronegativity: {
    keywords: ["electronegativity", "electronegative"],
    sort: (a, b) =>
      (parseFloat(b.electronegativity) || 0) -
      (parseFloat(a.electronegativity) || 0),
  },
  atomicMass: {
    keywords: ["atomic mass", "heaviest", "lightest"],
    sort: (a, b) =>
      (parseFloat(b.atomicMass) || 0) - (parseFloat(a.atomicMass) || 0),
  },
};
...
function parsePeriodGroup(msg) {
  return {
    period: msg.match(/period\s+(\d+)/i)
      ? parseInt(msg.match(/period\s+(\d+)/i)[1])
      : null,
    group: msg.match(/group\s+(\d+)/i)
      ? parseInt(msg.match(/group\s+(\d+)/i)[1])
      : null,
  };
}
function resolveRelevantElements(message) {
  ...
  if (period !== null || group !== null) {
    const f = ELEMENTS.filter(
      (el) =>
        (period === null || el.period === period) &&
        (group === null || el.group === group),
    );
    return f.length > 0 ? f : [];
  }
  for (const el of ELEMENTS) {
    if (msg.includes(el.name.toLowerCase())) found.set(el.atomicNumber, el);
    if (new RegExp(`\\b${el.symbol.toLowerCase()}\\b`).test(msg))
      found.set(el.atomicNumber, el);
  }
  for (const [cat, syns] of Object.entries(CATEGORY_SYNONYMS))
    if (syns.some((s) => msg.includes(s)))
      ELEMENTS.filter((el) => el.category === cat).forEach((el) =>
        found.set(el.atomicNumber, el),
      );
...
function parseGroqRateLimitError(err) {
  ...
  function getRetryTime(str) {
    ...
    const m = str.match(/(\d+)m/),
      s = str.match(/([\d.]+)s/);
    const totalMs =
      ((m ? parseInt(m[1]) : 0) * 60 + (s ? parseFloat(s[1]) : 0)) * 1000;
    ...
    if (err?.status === 429) {
      ...
      return res.status(429).json({
        error: isDaily
          ...
          : "Per-minute token limit reached.",
