...
class RateLimiter {
  ...
  getDynamicMaxTokens() {
    const pct = (QUOTA.DAILY_TOKENS - this.dailyTokensUsed) / QUOTA.DAILY_TOKENS;
    if (pct > 0.60) return QUOTA.BASE_MAX_TOKENS;
    if (pct > 0.30) return Math.floor(QUOTA.BASE_MAX_TOKENS * 0.70);
    if (pct > 0.10) return Math.floor(QUOTA.BASE_MAX_TOKENS * 0.45);
  ...
  check(sessionId) {
    ...
    if (sess.lastMessageAt > 0 && msSinceLast < QUOTA.MESSAGE_COOLDOWN_MS)
      return { code: 'COOLDOWN', waitMs: QUOTA.MESSAGE_COOLDOWN_MS - msSinceLast,
        message: `Please wait ${((QUOTA.MESSAGE_COOLDOWN_MS - msSinceLast)/1000).toFixed(1)}s.` };

    if (sess.tokensUsed >= QUOTA.SESSION_TOKEN_CAP)
      return { code: 'SESSION_CAP', message: 'Session token limit reached. Refresh to start a new session.' };
    ...
    if (now - gw.windowStart >= 60_000) { gw.count = 0; gw.windowStart = now; }
    if (gw.count >= QUOTA.GLOBAL_RPM)
      return { code: 'GLOBAL_RATE_LIMIT', message: 'Server is busy. Try again in a few seconds.' };

    const uw = sess.minuteWindow;
    if (now - uw.windowStart >= 60_000) { uw.count = 0; uw.windowStart = now; }
    if (uw.count >= QUOTA.USER_RPM)
      return { code: 'USER_RATE_LIMIT', message: `Too many messages. Limit is ${QUOTA.USER_RPM}/min.` };
  ...
  getSessionMeta(sessionId) {
    ...
    return {
      ...
      dailyTokensRemaining: Math.max(0, QUOTA.DAILY_TOKENS - this.dailyTokensUsed),
...
function resolveRelevantElements(message) {
  ...
  for (const [cat, syns] of Object.entries(CATEGORY_SYNONYMS))
    if (syns.some(s => msg.includes(s))) ELEMENTS.filter(el=>el.category===cat).forEach(el=>found.set(el.atomicNumber,el));
  const r = [...found.values()];
  return r.length > 0 ? r : ELEMENTS.map(e=>({ name:e.name,symbol:e.symbol,atomicNumber:e.atomicNumber,category:e.category??'',period:e.period,group:e.group }));
}
...
function buildSystemPrompt(elementData) {
  ...
  return `You are ChemBot, a chemistry assistant for the PeriodicX periodic table app.
CORE RULES:
...
4. Use ONLY the data below.
5. NON-CHEMISTRY: Reply ONLY: "I'm ChemBot — I only answer questions about chemical elements."
...
export default async function handler(req, res) {
  ...
  // Internal rate-limit check (before calling Groq)
  ...
  try {
    ...
    console.error('[ChemBot] Groq error status:', err?.status, err?.message);

    if (err?.status === 429) {
      const rateLimit = parseGroqRateLimitError(err);
      const isDaily   = rateLimit.type === 'daily';
      return res.status(429).json({
        error:     isDaily ? 'Daily token quota reached.' : 'Per-minute token limit reached.',
        code:      isDaily ? 'GROQ_TPD_LIMIT' : 'GROQ_TPM_LIMIT',
        rateLimit,                  // ← structured data for the UI card
      });
    }
